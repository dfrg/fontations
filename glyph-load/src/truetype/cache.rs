...
impl Cache {
    ...
    pub fn prepare(
        ...
        let max_twilight = data.max_twilight as usize + 4;
        self.twilight.resize(max_twilight * 3, Point::default());
        self.twilight_tags.resize(max_twilight, 0);
        self.stack.resize(data.max_stack as usize, 0);
        let font_entry = self.find_font(font_id);
        if !font_entry.0 {
            let max_fdefs = data.max_function_defs as usize;
            let max_idefs = data.max_instruction_defs as usize;
        ...
        if !size_entry.0 {
            ...
            let cvt_len = data.cvt.len();
            size.store.clear();
            size.store.resize(cvt_len + data.max_storage as usize, 0);
            data.scale_cvt(Some(scale), &mut size.store);
            run_prep = true;
        } else {
        ...
        if run_fpgm | run_prep {
            ...
            if run_fpgm {
                ...
                if !hinter.run_fpgm(&mut state, data.fpgm) {
                    return None;
                }
            }
            if run_prep {
                size.state = HinterState::default();
                if !hinter.run_prep(&mut size.state, mode, data.fpgm, data.prep, ppem, scale) {
                    return None;
                }
            }
        }
        let size = if size_entry.1 == !0 {
            ...
            &self.sizes[size_entry.1]
        };
        if !size.state.hinting_enabled() {
            return None;
        }
        if font_entry.1 != !0 {
    ...
    pub fn hint(
        ...
        let result = hinter.run(&mut size.state, data.fpgm, data.prep, ins, is_composite);
